...
import cn.bio.server.util.PacketParserUtil;
...
public class Client implements Comparable<Client> {
    ...
    public void onConnect(Socket socket) {
        ...
        this.socket = socket;
        logger.debug(TAG, "用户IP/port：" + getUserIP() + ":" + getUserPort());
    ...
    synchronized private void onDisconnected() {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                logger.debug(TAG, "onDisconnected() 1");

                protocol.onDisconnected();

                logger.debug(TAG, "onDisconnected() 2");
                ...
                logger.debug(TAG, "onDisconnected() 3");
                ...
                logger.debug(TAG, "onDisconnected() 4");
                ...
                logger.debug(TAG, "onDisconnected() 5");
            }
        }.start();
    ...
    public void sendMessage(BasicPacketParser selfData) {
        ...
        logger.debug(TAG, "send Msg to Client " + getClientID());
    ...
    public class ThreadReceiveHandler extends Thread {
        ...
        private void ackWrongMsg() {
            logger.debug(TAG, "%%%%%%%%%% ackWrongMsg (TODO)");
        }
        ...
        public void run() {
            while (isLoopRecAndSend) {
                ...
                try {
                    ...
                    logger.debug(TAG, "客户端 sends EOF. Socket is closed");
                    break;
                } catch (IOException e) {
                    e.printStackTrace();
                    break;
            ...
            logger.debug(TAG, "End of Thread: ThreadReceiveHandler");
        }
    ...
    public class ThreadSendHander extends Thread {
        ...
        public void run() {
                ...
                synchronized (queueToSend) {
                    ...
                    if (selfpro != null) {
                        try {
                            ...
                            logger.debug(TAG, String.format("[Client %s] going a message", getClientID()));
            ...
            logger.debug(TAG, "End of Thread: ThreadSendHander");
        }
    }
